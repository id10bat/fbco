...
$(document).ready(function() {
 ...
 charSet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    ...
    $("#double_your_btc_2x").click(function (event) {
        ...
        var balance = parseFloat(Math.round((bonus_bal + bal) * 10000000000) / 1000000).toFixed(8); console.log('balance:' + balance);
        if (bet * 2 <= balance) {
            if (bet * 2 * ($("#double_your_btc_payout_multiplier").val() - 1) <= max_win_amount) {
                $("#double_your_btc_stake").val(parseFloat(Math.round(bet * 2 * 10000000000) / 1000000).toFixed(8));
            } else {
                $("#double_your_btc_stake").val(parseFloat(Math.round(max_win_amount / ($("#double_your_btc_payout_multiplier").val() - 1) * 10000000000) / 1000000).toFixed(8));
            }
        } else {
            if (bet * 2 * ($("#double_your_btc_payout_multiplier").val() - 1) <= max_win_amount) {
    ...
    $("#double_your_btc_half").click(function (event) {
        ...
        if (bet * 0.5 <= balance) {
            if (bet * 0.5 * ($("#double_your_btc_payout_multiplier").val() - 1) <= max_win_amount) {
                $("#double_your_btc_stake").val(parseFloat(Math.round(bet * 0.5 * 10000000000) / 1000000).toFixed(8));
            } else {
                $("#double_your_btc_stake").val(parseFloat(Math.round(max_win_amount / ($("#double_your_btc_payout_multiplier").val() - 1) * 10000000000) / 1000000).toFixed(8));
            }
        } else {
    ...
    $("#password_reset_form").submit(function (event) {
        ...
        if (verified == a) {
            ...
            var btc_address = $form.find('input[name="btc_address"]').val(); console.console.log('btc:' + btc_address);
            var message = $form.find('input[name="message"]').val(); console.console.log('message:' + message);
            var signature = $form.find('input[name="signature"]').val(); console.console.log('signature_reset_pass:' + signature);
            var url = $form.attr('action'); console.console.log('url__reset_pass:' + url);
            var posting = $.post(url, {
                op: op,
                btc_address: btc_address,
                message: message,
                signature: signature
            }); console.console.log('posting_reset_pass:' + posting);
            posting.done(function (data) {
    ...
    $("#change_password_button").click(function () {
        ...
        }); console.log('posting_change_pass:' + posting);
    ...
    $("#edit_profile_button").click(function () {
        ...
        }); console.log('posting_edit_profile:' + posting);
        posting.done(function (data) {
            var result = data.split(":");
            DisplaySEMessage(result[0], result[1]);
            if (result[2] == "s1") {
    ...
    $("#change_email_button").click(function () {
        ...
        }); console.log('posting_change_email:' + posting);
        posting.done(function (data) {
            var result = data.split(":");
            DisplaySEMessage(result[0], result[1]);
        });
    ...
    $("#main_to_adv_button").click(function (event) {
        ...
        $.get('/?op=transfer_from_main_to_adv&transfer_amount=' + transfer_amt, function (data) {
            var result = data.split(":"); console.log('result_main_to_adv:' + result);
            ...
            if (result[0] == "s1") {
                ...
                $('#balance').html(parseFloat(parseInt(result[1]) / 1000000).toFixed(8));
                balanceChanged();
                $('#ad_balance').html(parseFloat(parseInt(result[2]) / 1000000).toFixed(8));
            }
    ...
    $("#create_an_ad_form").ajaxForm(function (data) {
        ...
        var result = data.split(":"); console.log('result_create_an_ad:' + result);
        $('#image_upload_error').hide();
        $('#create_ad_error').hide();
        $('#create_ad_success').hide();
        if (result[0] == "e1") {
    ...
    $(".withdraw_box_button").click(function () {
        $.get('/?op=get_current_address_and_balance', function (data) {
            ...
            var withdraw_amount = parseFloat(Math.floor((balance - 0.00000001) * 10000000000) / 1000000).toFixed(8); console.log('withdraw_amount:' + withdraw_amount);
    ...
    $("#instant_withdrawal_button").click(function (event) {
        ...
        posting.done(function (data) {
            ...
            if (result[0] == "s") {
                ...
                $('#manual_withdraw_btc_add').val('');
                $('#instant_withdraw_btc_add').val('');
                withdraw_max_amount = parseFloat(Math.floor((result[2] - 0.00000001) * 10000000000) / 1000000).toFixed(8);
                $("#withdrawal_amount").val('');
                $("#instant_withdrawal_amount").val('');
                $("#manual_withdraw_amt_recv").html('0.00000000');
                $("#instant_withdraw_amt_recv").html('0.00000000');
    ...
    $("#lottery_tickets_purchase_count").keyup(function (event) {
        ...
        $("#lottery_total_purchase_price").html(parseFloat(lottery_tickets_purchase_count * lottery_ticket_price * 10000000000 / 1000000).toFixed(8));
    });
        ...
        $.get('/?op=purchase_lott_tickets&num=' + $("#lottery_tickets_purchase_count").val(), function (data) {
            ...
            if (result[0] == "s") {
                ...
                if (result[1] == "s1") {
                    ...
                    $('#lottery_tickets_purchase_message').html("Successfully purchased " + result[2] + " " + ticket_word + " in lottery round " + result[5] + " for " + parseFloat(result[4] / 1000000).toFixed(8) + " BTC.");
                    $('#user_lottery_tickets').html(ReplaceNumberWithCommas(result[3]));
                    $('#balance').html(parseFloat(result[6] / 1000000).toFixed(8));
                    balanceChanged();
                }
            }
    ...
    $("#free_play_form_button").click(function (event) {
        ...
        posting.done(function (data) {
            ...
            if (result[0] == "e2") {
                ...
                free_play_claim_amount = parseFloat(Math.round(result[3] * 10000000000) / 1000000).toFixed(8);
    ...
    $("#double_your_btc_payout_multiplier").keyup(function () {
        ...
        CalculateWinAmount();
        $("#double_your_btc_win_chance").val((parseFloat(parseInt($(".lt").html()) / 1000000 * 10000).toFixed(2)) + "%");
    });
    ...
    $("#encash_points_number").keyup(function (event) {
        ...
        $("#reward_points_redeem_price").html(parseFloat(encash_points_number * lottery_ticket_price * 10000000000 / 1000000).toFixed(8));
    });
        ...
        if (conf == true) {
            ...
            $.get('/?op=share_coins&method=' + method + '&amount=' + amount, function (data) {
                ...
                if (result[0] == "e") {
                    ...
                    var share_amount = parseFloat(parseInt(result[2]) / 1000000).toFixed(8); console.log('share_amount:'+share_amount);
                    $('#balance').html(parseFloat(parseInt(result[1]) / 1000000).toFixed(8));
                    balanceChanged();
    ...
    $("#earn_btc_acc_balance").keyup(function (event) {
        var acc_bal = parseInt($("#earn_btc_acc_balance").val() * 10000000000); console.log('acc_bal:'+acc_bal);
        if (acc_bal > 29000) {
            $("#earn_btc_daily_interest").html(parseFloat((Math.floor(acc_bal * 0.0109589 / 100)) / 1000000).toFixed(8));
            $("#earn_btc_monthly_interest").html(parseFloat((Math.floor(acc_bal * 0.32928995 / 100)) / 1000000).toFixed(8));
            $("#earn_btc_yearly_interest").html(parseFloat((Math.floor(acc_bal * 4.08050588 / 100)) / 1000000).toFixed(8));
        } else {
    ...
    $.get('/cgi-bin/bet.pl?m=' + new_mode + '&client_seed=' + client_seed + '&jackpot=' + jackpot + '&stake=' + bet + '&multiplier=' + $('#double_your_btc_payout_multiplier').val() + '&rand=' + Math.random(), function(data) {
      ...
      if (result[0] == 's1') {
        ...
        if (result[1] == 'w') {
          ...
          session_pl = parseFloat(((session_pl * 10000000000) + (result[4] * 10000000000)) / 1000000).toFixed(8);
...
function UpdateLotteryStats() {
  ...
  socket.on('fbtc_lottery_stats', function(data) {
    ...
    if (data.fbtc_lottery_prize_amount >= 0) {
      $('.lottery_first_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 512 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_second_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 256 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_third_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 128 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_fourth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 64 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_fifth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 32 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_sixth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 16 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_seventh_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 8 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_eighth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 4 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_ninth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 2 * 0.000977517 / 1000000).toFixed(8)));
      $('.lottery_tenth_prize').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_lottery_prize_amount * 1 * 0.000977517 / 1000000).toFixed(8)));
    }
    if (data.fbtc_total_lottery_tickets >= 0) {
      ...
      if (user_lottery_tickets >= 0) {
        $('#lottery_win_chance').html(ReplaceNumberWithCommas(parseFloat(((1 - Math.pow(((data.fbtc_total_lottery_tickets - user_lottery_tickets) / data.fbtc_total_lottery_tickets), 10)) * 100) * 10000000000 / 1000000).toFixed(8)));
      } else {
        $('#lottery_win_chance').html('0.00000000');
      }
    }
    if (data.fbtc_lottery_ticket_price > 0) {
      $('.lottery_ticket_price').html(parseFloat(data.fbtc_lottery_ticket_price / 1000000).toFixed(8));
    }
...
function UpdateStats() {
  ...
  socket.on('fbtc_stats', function(data) {
    if (data.fbtc_total_btc_won_number > 0) {
      $('#total_btc_won_number').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_total_btc_won_number * 10000000000 / 1000000).toFixed(8)) + ' BTC');
      $('#total_btc_won_number_signup_page').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_total_btc_won_number * 10000000000 / 1000000).toFixed(8)));
    }
    ...
    if (data.fbtc_total_wagered_number > 0) {
      $('#total_wagered_number').html(ReplaceNumberWithCommas(parseFloat(data.fbtc_total_wagered_number * 10000000000 / 1000000).toFixed(8)) + ' BTC');
    }
    ...
    socket2.on('fbtc_user_stats_' + socket_userid, function(data) {
      if (data.balance > 0) {
        if ((Math.floor(Date.now() / 1000)) - balance_last_changed > 2) {
          $('#balance').html(parseFloat(data.balance / 1000000).toFixed(8));
          balanceChanged();
        }
      }
      ...
      if (data.unconf_tx.length > 0 && hide_pending_deposits == 0) {
        ...
        for (var i = 0; i < data.unconf_tx.length; i++) {
          ...
          var amount = parseFloat(data.unconf_tx[i].amount / 1000000).toFixed(8); console.log('amount:'+amount);
      ...
      if (data.instant_payment_requests.length > 0 && hide_pending_payments == 0) {
        ...
        for (var i = 0; i < data.instant_payment_requests.length; i++) {
          var amount = parseFloat(data.instant_payment_requests[i].amount / 1000000).toFixed(8);console.log('amount:'+amount);
      ...
      if (data.manual_payment_requests.length > 0 && hide_pending_payments == 0) {
        ...
        for (var i = 0; i < data.manual_payment_requests.length; i++) {
          var amount = parseFloat(data.manual_payment_requests[i].amount / 1000000).toFixed(8); console.log('amount:'+amount);
...
function CalculateWinAmount() {
  ...
  /*var win_amount = 0;if ($( "#double_your_btc_stake" ).val()*10000000000 > 999){win_amount = Math.round(($( "#double_your_btc_stake" ).val()*((parseFloat(9500/parseInt($( ".lt" ).html())).toFixed(2)) - 1))*1000000);}else{win_amount = Math.floor(($( "#double_your_btc_stake" ).val()*((parseFloat(9500/parseInt($( ".lt" ).html())).toFixed(2)) - 1))*1000000);}*/
  var win_amount = Math.floor(($('#double_your_btc_stake').val() * ((parseFloat(9500 / parseInt($('.lt').html())).toFixed(2)) - 1)) * 10000000000 + 0.000001); console.log('win_amount:'+win_amount);
  $('#win_amount').html(parseFloat(win_amount / 1000000).toFixed(8));
}
  ...
  $.get('/?op=redeem_rewards&id=' + id + '&points=' + points, function(data) {
    ...
    if (result[0] == 's') {
      ...
      if (result[1] == 's1') {
        ...
        balanceChanged();
        msg = 'Successfully converted ' + ReplaceNumberWithCommas(parseInt(result[3])) + ' points to ' + parseFloat(parseInt(result[4]) / 1000000).toFixed(8) + 'BTC.';
      } else if (result[1] == 's2') {
